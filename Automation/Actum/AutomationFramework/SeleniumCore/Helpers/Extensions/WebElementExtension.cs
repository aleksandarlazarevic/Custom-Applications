...
namespace SeleniumCore.Helpers.Extensions
{
    public static class WebElementExtension
    {
        ...
        public static void WaitForControlToBeDisplayed(By locator, string elementName, uint timeoutInSeconds = 60)
        {
            WebDriverWait wait = new WebDriverWait(UIDriver.WebDriver, TimeSpan.FromSeconds(timeoutInSeconds));
            wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementIsVisible(locator));
        }

        public static void WaitForControlToDisapear(By locator, string elementName, uint timeoutInSeconds = 60)
        {
            WebDriverWait wait = new WebDriverWait(UIDriver.WebDriver, TimeSpan.FromSeconds(timeoutInSeconds));
            wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.InvisibilityOfElementLocated(locator));
        }

        public static void WaitForControlToDisapear(this IWebElement element, string elementName, int numberOfRetries = 5)
        {
            int retry = 0;
            ...
            {
                throw new Exception(String.Format("[EXCEPTION] - Element:[{0}] did not disapear from the page ", elementName));
            }
            else
            {
            }
        ...
        public static bool DoesControlExist(By locator, uint timeoutInSeconds = 60)
        {
            UIDriver.WebDriver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(timeoutInSeconds);
            IWait<IWebDriver> wait = new WebDriverWait(UIDriver.WebDriver, TimeSpan.FromSeconds(timeoutInSeconds));
            wait.IgnoreExceptionTypes
