...
namespace SeleniumEngine.Base
{
    public class BasePage
    {
        ...
        #endregion
        #region Getters
        public static By GetElementBy(Type pageType, string propertyName)
        {
            By getter = null;
        ...
        public T ImplicitlyWaitForPageToBeReady<T>(int seconds = 1) where T : BasePage
        {
            ...
            return WebDriverFactory.WebDriver.GetPage<T>();
        }
        ...
        private bool HasBecameVisible(By locator, string description, bool isProcessing, uint timeout)
        {
            ...
            {
                IWebElement element = WebDriverFactory.WebDriver.FindElementEx(locator, 0);
        ...
        private bool HasBecameInvisible(By locator, string description, uint timeout)
        {
            ...
            while (time.Elapsed < TimeSpan.FromSeconds(timeout))
            {
                if (WebDriverFactory.WebDriver.IsDisplayed(locator, 0))
                {
                    Thread.Sleep(250);
                    continue;
                }
